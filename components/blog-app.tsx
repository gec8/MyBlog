...
import { DraftKind, DraftSort, draftKindLabel, filterAndSortDrafts } from "@/components/admin/draft-utils";
  ...
  return <article className={`post-card ${featured ? "featured" : ""}`}>
    <button className="card-hit" onClick={() => go(`post/${post.slug}`)} aria-label={`阅读：${post.title}`}>
      <div className={`post-cover ${coverTone(post.category)} ${post.coverImage ? "has-image" : ""}`}>
        {post.coverImage && <><img src={post.coverImage} alt="" style={{ objectPosition: post.coverPosition || "50% 50%", filter: `brightness(${post.coverBrightness ?? 100}%)` }} loading={featured ? "eager" : "lazy"} decoding="async" fetchPriority={featured ? "high" : "auto"} onError={(event) => { event.currentTarget.hidden = true; event.currentTarget.parentElement?.classList.add("image-failed"); }} /><i className="cover-overlay" style={{ opacity: (post.coverOverlay ?? 12) / 100 }} /></>}
...
function Article({ post, posts, settings }: { post: Post; posts: Post[]; settings: SiteSettings }) {
  ...
  return <>
    ...
    <main className="article-layout">
      ...
      <div className={`article-cover ${coverTone(post.category)} ${post.coverImage ? "has-image" : ""}`}>{post.coverImage && <><img src={post.coverImage} alt="" style={{ objectPosition: post.coverPosition || "50% 50%", filter: `brightness(${post.coverBrightness ?? 100}%)` }} loading="eager" decoding="async" fetchPriority="high" onError={(event) => { event.currentTarget.hidden = true; event.currentTarget.parentElement?.classList.add("image-failed"); }} /><i className="cover-overlay" style={{ opacity: (post.coverOverlay ?? 12) / 100 }} /></>}<span>{post.category}</span><b>{post.date}</b>{post.coverCredit && post.coverCreditUrl && <a className="cover-credit" href={post.coverCreditUrl} target="_blank" rel="noreferrer">图片：{post.coverCredit} / Pexels</a>}</div>
...
type Draft = { title: string; slug: string; excerpt: string; category: string; author: string; coverImage: string; coverCredit: string; coverCreditUrl: string; coverPosition: string; coverBrightness: number; coverOverlay: number; coverPexelsId?: number; content: string };
...
const emptyDraft: Draft = { title: "", slug: "", excerpt: "", category: "随笔", author: "Neko", coverImage: "", coverCredit: "", coverCreditUrl: "", coverPosition: "50% 50%", coverBrightness: 100, coverOverlay: 12, content: "## 从这里开始\n\n写下你的正文。" };
...
function Admin({ posts, initialSettings }: { posts: Post[]; initialSettings: SiteSettings }) {
  ...
  const [draftDeleteTarget, setDraftDeleteTarget] = useState<SavedDraft | null>(null);
  ...
  const [coverPickerTab, setCoverPickerTab] = useState<"media" | "pexels">("pexels");
  ...
  useEffect(() => { if (panel === "media" && connected) void loadRepoMedia(); }, [panel, connected]);
  ...
  const preview = useMemo<Post>(() => ({
    ...
    author: draft.author || "Neko", coverImage: draft.coverImage.trim() || undefined, coverCredit: draft.coverCredit.trim() || undefined, coverCreditUrl: draft.coverCreditUrl.trim() || undefined, coverPosition: draft.coverPosition, coverBrightness: draft.coverBrightness, coverOverlay: draft.coverOverlay, coverPexelsId: draft.coverPexelsId, date: new Date().toISOString().slice(0, 10),
  ...
  const suggestedCoverQuery = useMemo(() => coverKeywords(draft.title, draft.category), [draft.title, draft.category]);
  ...
  async function loadRepoMedia(showResult = false) { try { const folders: Array<{ folder: string; type: "image" | "audio" }> = [{ folder: "images", type: "image" }, { folder: "audio", type: "audio" }]; const results = await Promise.all(folders.map(async ({ folder, type }) => { const query = new URLSearchParams({ ref: config.branch.trim(), t: String(Date.now()) }); const response = await fetch(`${contentsApi(`public/${folder}`)}?${query}`, { headers: headers(), cache: "no-store" }); if (response.status === 404) return []; if (!response.ok) throw new Error(await githubError(response, "媒体目录读取失败。")); const files = await response.json() as Array<{ name: string; path: string; sha: string; size: number; type: string }>; if (!Array.isArray(files)) return []; return files.filter((file) => file.type === "file").map((file) => ({ name: file.name, path: file.path, sha: file.sha, size: file.size, type, url: `./${folder}/${file.name}` })); })); setRepoMedia(results.flat()); if (showResult) { setState("success"); setMessage(`媒体列表已刷新，共 ${results.flat().length} 个资源。`); } } catch (error) { setState("error"); setMessage(error instanceof TypeError ? "无法连接 GitHub，请检查网络、浏览器隐私拦截或令牌后重试。" : error instanceof Error ? error.message : "媒体读取失败。"); } }

  async function uploadMediaLibrary(event: ChangeEvent<HTMLInputElement>) { const files = Array.from(event.target.files ?? []); if (!files.length) return; setState("uploading"); setMessage(`正在上传 ${files.length} 个媒体文件…`); let uploaded = 0; try { for (const file of files) { const type = mediaTypeOf(file); if (!type) throw new Error(`不支持 ${file.name} 的文件格式，请选择常用图片或音频。`); const limit = type === "audio" ? 15 * 1024 * 1024 : 5 * 1024 * 1024; if (!file.size) throw new Error(`${file.name} 是空文件，无法上传。`); if (file.size > limit) throw new Error(`${file.name} 超过 ${type === "audio" ? "15MB" : "5MB"} 限制。`); const prepared = type === "image" ? await prepareImage(file) : { content: await fileToBase64(file), extension: file.name.split(".").pop()?.toLowerCase() || "mp3" }; const name = mediaFileName(file.name, prepared.extension); const folder = type === "audio" ? "audio" : "images"; const path = `public/${folder}/${name}`; const response = await fetch(contentsApi(path), { method: "PUT", headers: { ...headers(), "Content-Type": "application/json" }, body: JSON.stringify({ message: `upload: ${name}`, content: prepared.content, branch: config.branch.trim() }) }); if (!response.ok) throw new Error(await githubError(response, `${file.name} 上传失败。`)); const result = await response.json() as { content?: { name?: string; path?: string; sha?: string; size?: number } }; const saved: RepoMedia = { name: result.content?.name || name, path: result.content?.path || path, sha: result.content?.sha || "", size: result.content?.size || file.size, type, url: `./${folder}/${name}` }; setRepoMedia((current) => [saved, ...current.filter((item) => item.path !== saved.path)]); uploaded += 1; setMessage(`已上传 ${uploaded}/${files.length}：${file.name}`); } setState("success"); setMessage(`${uploaded} 个媒体文件已上传并加入列表。`); } catch (error) { setState("error"); setMessage(`${uploaded ? `已成功 ${uploaded} 个；` : ""}${error instanceof Error ? error.message : "媒体上传失败。"}`); } finally { event.target.value = ""; } }
  ...
  async function uploadImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      updateDraft({ coverImage: `./images/${name}`, coverCredit: "", coverCreditUrl: "", coverPexelsId: undefined, coverPosition: "50% 50%", coverBrightness: 100, coverOverlay: 12 }); setState("success"); setMessage("图片已上传并设为封面。");
  ...
  async function searchPexels(page = 1, random = false, queryOverride?: string) {
    ...
    try {
      ...
      const searchTerm = (queryOverride ?? pexelsQuery).trim() || suggestedCoverQuery;
      const endpoint = searchTerm ? `https://api.pexels.com/v1/search?query=${encodeURIComponent(searchTerm)}&orientation=landscape&size=large&per_page=12&page=${targetPage}` : `https://api.pexels.com/v1/curated?per_page=12&page=${targetPage}`;
      ...
      const freshPhotos = photos.filter((photo) => photo.id !== draft.coverPexelsId);
      setPexelsPhotos(random && freshPhotos.length ? [freshPhotos[Math.floor(Math.random() * freshPhotos.length)]] : photos); setPexelsPage(targetPage);
  ...
  async function usePexelsPhoto(photo: PexelsPhoto) {
    ...
    try {
      const reused = repoMedia.find((item) => item.type === "image" && item.name.startsWith(`pexels-${photo.id}.`));
      if (reused) { updateDraft({ coverImage: reused.url, coverCredit: photo.photographer, coverCreditUrl: photo.url || photo.photographer_url, coverPexelsId: photo.id, coverPosition: "50% 50%", coverBrightness: 100, coverOverlay: 12 }); setShowPexels(false); setState("success"); setMessage("已从媒体库复用这张 Pexels 封面。"); return; }
      ...
      const name = `pexels-${photo.id}.${prepared.extension.toLowerCase()}`; const path = `public/images/${name}`;
      const existing = await fetch(`${contentsApi(path)}?ref=${encodeURIComponent(config.branch.trim())}&t=${Date.now()}`, { headers: headers(), cache: "no-store" });
      let saved: RepoMedia;
      if (existing.ok) { const file = await existing.json() as { sha: string; size: number }; saved = { name, path, sha: file.sha, size: file.size, type: "image", url: `./images/${name}` }; }
      else { const response = await fetch(contentsApi(path), { method: "PUT", headers: { ...headers(), "Content-Type": "application/json" }, body: JSON.stringify({ message: `upload: ${name}`, content: prepared.content, branch: config.branch.trim() }) }); if (!response.ok) throw new Error(await githubError(response, "网络封面保存失败。")); const result = await response.json() as { content?: { sha?: string; size?: number } }; saved = { name, path, sha: result.content?.sha || "", size: result.content?.size || blob.size, type: "image", url: `./images/${name}` }; }
      setRepoMedia((current) => [saved, ...current.filter((item) => item.path !== path)]);
      updateDraft({ coverImage: saved.url, coverCredit: photo.photographer, coverCreditUrl: photo.url || photo.photographer_url, coverPexelsId: photo.id, coverPosition: "50% 50%", coverBrightness: 100, coverOverlay: 12 });
  ...
  async function uploadInlineImage(event: DragEvent<HTMLTextAreaElement>) {
    ...
    try { const prepared = await prepareImage(file); const name = mediaFileName(file.name, prepared.extension); const body = JSON.stringify({ message: `upload: ${name}`, content: prepared.content, branch: config.branch.trim() }); const response = await fetch(contentsApi(`public/images/${name}`), { method: "PUT", headers: { ...headers(), "Content-Type": "application/json" }, body }); if (!response.ok) throw new Error("正文图片上传失败。"); updateDraft({ content: `${draft.content.trimEnd()}\n\n![${file.name}](./images/${name})\n` }); setState("success"); setMessage("图片已插入正文末尾。"); }
    catch (error) { setState("error"); setMessage(error instanceof Error ? error.message : "图片上传失败。"); }
  }
  ...
  return <div className="admin-shell">
    ...
    {!connected ? <main className="connect-layout">
      ...
      <section className="workspace-main">
        ...
        {panel === "media" && <section className={`manage-panel media-library ${managingMedia ? "is-managing" : ""}`}>
          <header><div><h2>媒体资源</h2><p>统一管理仓库中的图片与音频，使用中的文件会受到删除保护。</p></div><span><b>{repoMedia.length} 个资源</b><input ref={mediaUploadRef} hidden multiple type="file" accept="image/*,audio/*" onChange={uploadMediaLibrary} /><button onClick={() => mediaUploadRef.current?.click()} disabled={state === "uploading"}><Upload />上传</button><button onClick={() => void loadRepoMedia(true)} disabled={state === "uploading"}><RefreshCw />刷新</button><button onClick={() => { setManagingMedia(!managingMedia); setSelectedMedia([]); }}>{managingMedia ? "完成" : "管理"}</button></span></header>
          <div className="media-filters" role="group" aria-label="媒体类型筛选">{(["all", "image", "audio", "unused"] as const).map((filter) => <button key={filter} className={mediaFilter === filter ? "active" : ""} onClick={() => { setMediaFilter(filter); setSelectedMedia([]); }}>{filter === "all" ? "全部" : filter === "image" ? "图片" : filter === "audio" ? "音频" : "未引用"}</button>)}</div>
          {managingMedia && <div className="media-batch"><label><input type="checkbox" checked={allMedia.length > 0 && allMedia.every((item) => selectedMedia.includes(item.path))} onChange={(event) => setSelectedMedia(event.target.checked ? allMedia.map((item) => item.path) : [])} />全选当前结果</label><span>已选择 {selectedMedia.length} 个</span><button disabled={!selectedMedia.length} onClick={deleteSelectedMedia}><Trash2 />删除未引用项</button></div>}
          {allMedia.length ? <div className="media-grid">{allMedia.map((item) => <article key={item.path} className={selectedMedia.includes(item.path) ? "selected" : ""}>
            {managingMedia && <label className="media-check"><input type="checkbox" checked={selectedMedia.includes(item.path)} onChange={(event) => setSelectedMedia((current) => event.target.checked ? [...current, item.path] : current.filter((path) => path !== item.path))} /><span className="sr-only">选择 {item.name}</span></label>}
            <span className={`media-thumb ${item.type}`}>{item.type === "image" ? <img src={item.url} alt="" loading="lazy" /> : <Music2 />}</span>
            <div className="media-info"><b title={item.name}>{item.name}</b><small>{item.type === "image" ? "图片" : "音频"} · {item.size < 1024 * 1024 ? `${Math.max(1, Math.round(item.size / 1024))} KB` : `${(item.size / 1024 / 1024).toFixed(1)} MB`}</small><p className={item.posts.length ? "in-use" : "unused"}>{item.posts.length ? `使用中 · ${item.posts.length} 篇文章：${item.posts.join("、")}` : "未被文章引用，可安全删除"}</p></div>
            <details className="media-actions"><summary aria-label={`管理 ${item.name}`}>•••</summary><div><button onClick={() => { const markdown = item.type === "audio" ? `\n\n@[audio:文章音频](${item.url})\n` : `\n\n![图片说明](${item.url})\n`; updateDraft({ content: `${draft.content.trimEnd()}${markdown}` }); setPanel("editor"); }}><Copy />插入正文</button><button onClick={() => navigator.clipboard.writeText(item.url)}><Link2 />复制地址</button><a href={item.url} target="_blank" rel="noreferrer"><ArrowUpRight />查看原文件</a><button className="danger" disabled={Boolean(item.posts.length)} title={item.posts.length ? `被 ${item.posts.length} 篇文章引用，不能删除` : "删除未引用文件"} onClick={() => deleteRepoMedia(item)}><Trash2 />{item.posts.length ? "使用中，不能删除" : "删除文件"}</button></div></details>
          </article>)}</div> : <div className="list-empty"><ImagePlus /><p>{repoMedia.length ? "当前筛选下没有资源" : "仓库中还没有图片或音频"}</p><button onClick={() => mediaUploadRef.current?.click()}>上传第一个资源</button></div>}
        </section>}

        {panel === "drafts" && <section className={`manage-panel draft-library ${managingDrafts ? "is-managing" : ""}`}><header><div><h2>本机草稿</h2><p>搜索、筛选和管理当前设备上的最近 20 份内容。</p></div><span><b>{visibleDrafts.length}/{savedDrafts.length} 份</b>{savedDrafts.length > 0 && <button onClick={() => { setManagingDrafts(!managingDrafts); setSelectedDraftIds([]); }}>{managingDrafts ? "完成" : "管理"}</button>}</span></header><div className="draft-filters"><label><Search/><input value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="搜索标题或正文"/></label><select value={draftKind} onChange={(event) => setDraftKind(event.target.value as DraftKind)}><option value="all">全部类型</option><option value="new">新文章</option><option value="published">文章修改</option></select><select value={draftSort} onChange={(event) => setDraftSort(event.target.value as DraftSort)}><option value="updated">最近更新</option><option value="oldest">最早更新</option><option value="title">按标题</option></select></div>{managingDrafts && <div className="draft-batch"><label><input type="checkbox" checked={visibleDrafts.length > 0 && visibleDrafts.every((item) => selectedDraftIds.includes(item.id))} onChange={(event) => setSelectedDraftIds(event.target.checked ? Array.from(new Set([...selectedDraftIds, ...visibleDrafts.map((item) => item.id)])) : selectedDraftIds.filter((id) => !visibleDrafts.some((item) => item.id === id)))}/>全选当前结果</label><span>已选择 {selectedDraftIds.length} 份</span><button disabled={!selectedDraftIds.length} onClick={batchDeleteDrafts}><Trash2 />删除所选</button></div>}{visibleDrafts.length ? <div>{visibleDrafts.map((item) => <article className={selectedDraftIds.includes(item.id) ? "selected" : ""} key={item.id}>{managingDrafts && <label className="draft-check"><input type="checkbox" checked={selectedDraftIds.includes(item.id)} onChange={(event) => setSelectedDraftIds((current) => event.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))}/><span/></label>}<button className="draft-open" onClick={() => managingDrafts ? setSelectedDraftIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id]) : (() => { setDraft({ ...emptyDraft, ...item.draft }); setActiveDraftId(item.id); setEditingId(item.id.startsWith("post-") ? item.id.slice(5) : null); setDirty(false); setPanel("editor"); })()}><span><Save /></span><div><b>{item.draft.title || "未命名草稿"}</b><small>{draftKindLabel(item.id)} · {new Date(item.savedAt).toLocaleString("zh-CN")} · {item.draft.content.replace(/\s/g, "").length} 字</small></div></button>{!managingDrafts && <details className="draft-actions"><summary aria-label="草稿操作">•••</summary><div><button onClick={() => { setDraft({ ...emptyDraft, ...item.draft }); setActiveDraftId(item.id); setPanel("editor"); }}><Pencil />继续编辑</button><button onClick={() => duplicateDraft(item)}><Copy />复制草稿</button><button className="danger" onClick={() => setDraftDeleteTarget(item)}><Trash2 />删除草稿</button></div></details>}</article>)}</div> : <div className="list-empty"><Save /><p>{savedDrafts.length ? "没有符合条件的草稿" : "还没有保存的草稿"}</p>{!savedDrafts.length && <Button onClick={newPost}>开始写作</Button>}</div>}</section>}
        ...
        {panel === "editor" && <div className="editor-workspace">
          ...
          <form className={`editor-panel ${mobilePreview ? "mobile-hidden" : ""}`} onSubmit={(event) => { event.preventDefault(); setShowPublishCheck(true); }}>
            <section className="write-section">
              ...
              <Field label="封面图片"><div className="cover-source-tabs"><button type="button" onClick={() => imageRef.current?.click()} disabled={state === "uploading"}><Upload />自己上传</button><button type="button" onClick={() => { setCoverPickerTab("media"); setShowPexels(true); void loadRepoMedia(); }}><ImagePlus />媒体库</button><button type="button" onClick={() => { setCoverPickerTab("pexels"); setShowPexels(true); if (!pexelsPhotos.length && pexelsKey) void searchPexels(1); }}><Search />网络图库</button></div><div className="cover-field"><Input value={draft.coverImage} onChange={(e) => updateDraft({ coverImage: e.target.value, coverCredit: "", coverCreditUrl: "", coverPexelsId: undefined })} placeholder="封面地址，也可以使用上方三种来源" /><Button type="button" variant="outline" onClick={() => { setCoverPickerTab("pexels"); void searchPexels(1, true); }}><Shuffle />随机一张</Button><input ref={imageRef} className="file-input" type="file" accept="image/*" onChange={(event) => void uploadImage(event)} /></div>{draft.coverImage && <><div className="cover-preview"><img src={draft.coverImage} alt="封面预览" style={{ objectPosition: draft.coverPosition, filter: `brightness(${draft.coverBrightness}%)` }} /><i className="cover-overlay" style={{ opacity: draft.coverOverlay / 100 }} /><button type="button" onClick={() => updateDraft({ coverImage: "", coverCredit: "", coverCreditUrl: "", coverPexelsId: undefined })}><X />移除封面</button>{draft.coverCredit && <a href={draft.coverCreditUrl} target="_blank" rel="noreferrer">图片：{draft.coverCredit} / Pexels</a>}</div><div className="cover-adjust"><div><b>图片焦点</b><span className="focus-grid">{[["0% 0%","↖"],["50% 0%","↑"],["100% 0%","↗"],["0% 50%","←"],["50% 50%","•"],["100% 50%","→"],["0% 100%","↙"],["50% 100%","↓"],["100% 100%","↘"]].map(([position,label]) => <button type="button" key={position} className={draft.coverPosition === position ? "active" : ""} onClick={() => updateDraft({ coverPosition: position })} aria-label={`设置焦点 ${label}`}>{label}</button>)}</span></div><label><span>亮度 <b>{draft.coverBrightness}%</b></span><input type="range" min="70" max="115" value={draft.coverBrightness} onChange={(event) => updateDraft({ coverBrightness: Number(event.target.value) })} /></label><label><span>遮罩 <b>{draft.coverOverlay}%</b></span><input type="range" min="0" max="45" value={draft.coverOverlay} onChange={(event) => updateDraft({ coverOverlay: Number(event.target.value) })} /></label></div></>}<small className="token-hint">支持 16:9 预览、焦点定位和显示效果调整；网络图片会保存到 GitHub</small></Field>
    ...
    {showPexels && <div className="modal-backdrop pexels-backdrop" role="presentation" onMouseDown={() => setShowPexels(false)}><section className={`pexels-picker ${coverPickerTab === "media" ? "is-media" : "is-network"}`} role="dialog" aria-modal="true" aria-labelledby="pexels-title" onMouseDown={(event) => event.stopPropagation()}><header><div><small>COVER LIBRARY</small><h2 id="pexels-title">选择文章封面</h2><p>复用已有图片，或从 Pexels 获取真实图片。</p></div><button onClick={() => setShowPexels(false)} aria-label="关闭"><X /></button></header><nav className="cover-picker-tabs"><button className={coverPickerTab === "media" ? "active" : ""} onClick={() => { setCoverPickerTab("media"); void loadRepoMedia(); }}><ImagePlus />媒体库</button><button className={coverPickerTab === "pexels" ? "active" : ""} onClick={() => setCoverPickerTab("pexels")}><Search />Pexels 网络图库</button></nav>{coverPickerTab === "media" && <div className="cover-media-grid">{repoMedia.filter((item) => item.type === "image").length ? repoMedia.filter((item) => item.type === "image").map((item) => <article key={item.path}><img src={item.url} alt={item.name} loading="lazy" /><div><b>{item.name}</b><button onClick={() => { updateDraft({ coverImage: item.url, coverCredit: "", coverCreditUrl: "", coverPexelsId: undefined, coverPosition: "50% 50%", coverBrightness: 100, coverOverlay: 12 }); setShowPexels(false); setMessage("已从媒体库选择封面。"); }}>使用此封面</button></div></article>) : <div className="pexels-empty"><ImagePlus /><p>媒体库还没有图片，请先上传或切换到网络图库</p></div>}</div>}<form onSubmit={(event) => { event.preventDefault(); void searchPexels(1); }}><label><Search /><input value={pexelsQuery} onChange={(event) => setPexelsQuery(event.target.value)} placeholder="搜索自然、城市、科技…" aria-label="搜索 Pexels 图片" /></label><button type="submit" disabled={pexelsLoading || !pexelsKey}>搜索</button><button type="button" disabled={pexelsLoading || !pexelsKey} onClick={() => void searchPexels(1, true)}><Shuffle />随机</button></form>{!pexelsKey && <div className="pexels-key"><KeyRound /><div><b>需要 Pexels API Key</b><small>密钥只保存在当前浏览器，不会进入网站仓库。</small></div><Input type="password" value={pexelsKey} onChange={(event) => savePexelsKey(event.target.value)} placeholder="粘贴 API Key" /></div>}{pexelsLoading ? <div className="pexels-loading"><LoaderCircle className="spin" /><p>正在获取真实图片…</p></div> : pexelsPhotos.length ? <div className="pexels-grid">{pexelsPhotos.map((photo) => <article key={photo.id}><img src={photo.src.medium} alt={photo.alt || `Pexels 摄影师 ${photo.photographer} 的图片`} loading="lazy" onError={(event) => { if (event.currentTarget.dataset.fallback) return; event.currentTarget.dataset.fallback = "1"; event.currentTarget.src = photo.src.large; }} /><div><a href={photo.url} target="_blank" rel="noreferrer">{photo.photographer} / Pexels</a><button onClick={() => void usePexelsPhoto(photo)}>使用此封面</button></div></article>)}</div> : <div className="pexels-empty"><ImagePlus /><p>{pexelsKey ? "搜索关键词，挑选一张真实封面" : "填写 API Key 后即可搜索图片"}</p></div>}<footer><a href="https://www.pexels.com" target="_blank" rel="noreferrer">Photos provided by Pexels</a>{coverPickerTab === "pexels" && pexelsPhotos.length > 1 && <span><button disabled={pexelsLoading || pexelsPage <= 1} onClick={() => void searchPexels(Math.max(1, pexelsPage - 1))}><ArrowLeft />上一页</button><button disabled={pexelsLoading} onClick={() => void searchPexels(pexelsPage + 1)}>下一页<ArrowRight /></button></span>}</footer></section></div>}
...
function mediaTypeOf(file: File): "image" | "audio" | null {
  if (file.type.startsWith("image/") || /\.(?:jpe?g|png|gif|webp|svg|avif)$/i.test(file.name)) return "image";
  if (file.type.startsWith("audio/") || /\.(?:mp3|m4a|aac|wav|ogg|oga|webm|flac)$/i.test(file.name)) return "audio";
  return null;
}

async function githubError(response: Response, fallback: string) {
  try { const detail = await response.json() as { message?: string }; return detail.message ? `${fallback} GitHub：${detail.message}` : fallback; }
