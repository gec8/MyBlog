...
function Admin({ posts, initialSettings }: { posts: Post[]; initialSettings: SiteSettings }) {
  ...
  async function loadRepoMedia() { try { const folders: Array<{ folder: string; type: "image" | "audio" }> = [{ folder: "images", type: "image" }, { folder: "audio", type: "audio" }]; const results = await Promise.all(folders.map(async ({ folder, type }) => { const response = await fetch(`${contentsApi(`public/${folder}`)}?ref=${encodeURIComponent(config.branch.trim())}`, { headers: headers() }); if (response.status === 404) return []; if (!response.ok) throw new Error("媒体目录读取失败。"); const files = await response.json() as Array<{ name: string; path: string; sha: string; size: number; type: string }>; return files.filter((file) => file.type === "file").map((file) => ({ ...file, type, url: `./${folder}/${file.name}` })); })); setRepoMedia(results.flat()); } catch (error) { setState("error"); setMessage(error instanceof Error ? error.message : "媒体读取失败。"); } }

  async function uploadMediaLibrary(event: ChangeEvent<HTMLInputElement>) { const files = Array.from(event.target.files ?? []); if (!files.length) return; setState("uploading"); setMessage(`正在上传 ${files.length} 个媒体文件…`); try { for (const file of files) { const type = file.type.startsWith("audio/") ? "audio" : file.type.startsWith("image/") ? "image" : null; if (!type) throw new Error(`不支持 ${file.name} 的文件格式。`); const limit = type === "audio" ? 15 * 1024 * 1024 : 5 * 1024 * 1024; if (file.size > limit) throw new Error(`${file.name} 超过大小限制。`); const prepared = type === "image" ? await prepareImage(file) : { content: await fileToBase64(file), extension: file.name.split(".").pop()?.toLowerCase() || "mp3" }; const name = mediaFileName(file.name, prepared.extension); const folder = type === "audio" ? "audio" : "images"; const response = await fetch(contentsApi(`public/${folder}/${name}`), { method: "PUT", headers: { ...headers(), "Content-Type": "application/json" }, body: JSON.stringify({ message: `upload: ${name}`, content: prepared.content, branch: config.branch.trim() }) }); if (!response.ok) throw new Error(`${file.name} 上传失败。`); } await loadRepoMedia(); setState("success"); setMessage(`${files.length} 个媒体文件已上传。`); } catch (error) { setState("error"); setMessage(error instanceof Error ? error.message : "媒体上传失败。"); } finally { event.target.value = ""; } }
    ...
    {!connected ? <main className="connect-layout">
      ...
      <section className="workspace-main">
        ...
        {panel === "media" && <section className={`manage-panel media-library ${managingMedia ? "is-managing" : ""}`}>
          <header><div><h2>媒体资源</h2><p>统一管理仓库中的图片与音频，使用中的文件会受到删除保护。</p></div><span><b>{repoMedia.length} 个资源</b><input ref={mediaUploadRef} hidden multiple type="file" accept="image/*,audio/*" onChange={uploadMediaLibrary} /><button onClick={() => mediaUploadRef.current?.click()} disabled={state === "uploading"}><Upload />上传</button><button onClick={() => { setManagingMedia(!managingMedia); setSelectedMedia([]); }}>{managingMedia ? "完成" : "管理"}</button></span></header>
