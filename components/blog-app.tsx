  ...
  return <div className="admin-shell">
    ...
    {!connected ? <main className="connect-layout">
      ...
      <section className="workspace-main">
        ...
        {panel === "drafts" && <section className="manage-panel draft-library"><header><div><h2>本机草稿</h2><p>自动保存最近 20 份内容，仅存放在当前设备。</p></div><b>{savedDrafts.length} 份</b></header>{savedDrafts.length ? <div>{savedDrafts.map((item) => <article key={item.id}><button className="draft-open" onClick={() => { setDraft({ ...emptyDraft, ...item.draft }); setActiveDraftId(item.id); setEditingId(item.id.startsWith("post-") ? item.id.slice(5) : null); setDirty(false); setPanel("editor"); }}><span><Save /></span><div><b>{item.draft.title || "未命名草稿"}</b><small>{new Date(item.savedAt).toLocaleString("zh-CN")} · {item.draft.content.replace(/\s/g, "").length} 字</small></div></button><details className="draft-actions"><summary aria-label="草稿操作">•••</summary><div><button onClick={() => { setDraft({ ...emptyDraft, ...item.draft }); setActiveDraftId(item.id); setPanel("editor"); }}><Pencil />继续编辑</button><button onClick={() => duplicateDraft(item)}><Copy />复制草稿</button><button className="danger" onClick={() => setDraftDeleteTarget(item)}><Trash2 />删除草稿</button></div></details></article>)}</div> : <div className="list-empty"><Save /><p>还没有保存的草稿</p><Button onClick={newPost}>开始写作</Button></div>}</section>}
